...
class UltimasTransmisiones():
    ...
    def ActualizaPantente(self, patente,pto1_latitud, pto1_longitud, pto1_fecha,pto1_hora, pto2_latitud, pto2_longitud, pto2_fecha, pto2_hora, servicio, sentido, brujula, servicioSentidoABordoDelBus, SOC, dentro_geo_pto1, dentro_geo_pto2, tracktec_latitud, tracktec_longitud, tracktec_fecha_hora):
        try:
            fechaHora = datetime.now().replace(microsecond=0) 
            #fecha = str(fechaHora.date())
            #hora  = str(fechaHora.time())
            parametros= (pto1_fecha,pto1_hora,pto1_latitud, pto1_longitud,pto2_fecha, pto2_hora,pto2_latitud, pto2_longitud,str(brujula),servicio, sentido, servicioSentidoABordoDelBus, SOC, dentro_geo_pto1, dentro_geo_pto2,tracktec_latitud,tracktec_longitud,tracktec_fecha_hora, patente)
